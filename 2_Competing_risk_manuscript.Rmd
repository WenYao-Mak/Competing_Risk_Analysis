...
#COMPETING RISK REGRESSION (SUBDISTRIBUTION HAZARD)
...
explanatory = c("Age2", "PtAssist", "dm", "brand", "frailty(site)") #reduced model stratified on site 

  comp %>% 
    #summary table 
  summary_factorlist(dependent_dss, explanatory, column = TRUE, fit_id = TRUE) %>% 
  
    #Cox PH multivariable for Peritonitis 
      ff_merge(
        comp %>% 
          coxphmulti(dependent_dss, explanatory) %>%
          fit2df(estimate_suffix = " (DSS CPH Peritonitis)")
      ) %>%
    #Cox PH multivariable for Technique failure 
      ff_merge(
        comp %>% 
          coxphmulti(dependent_dss2, explanatory) %>%
          fit2df(estimate_suffix = " (DSS CPH Technique Failure)")
      ) %>%
  
    #Fine & Gray Competing Risk model for Peritonitis 
      ff_merge(
        comp %>% 
          crrmulti(dependent_crr, explanatory) %>% 
          fit2df(estimate_suffix = " (CRR SH Peritonitis)")
      ) %>% 
    
    #Fine & Gray Competing Risk model for technique failure 
      ff_merge(
        comp %>% 
          crrmulti(dependent_crr2, explanatory) %>%
          fit2df(estimate_suffix = "(CRR SH Technique Failure")
      ) %>%
...
###################
### interaction ###    Add interaction term between age group and PD assistance 
###################
...
explanatory = c("Age2", "PtAssist", "dm", "brand", "frailty(site)") #Only one covariate at a time 
...
```{r PH_SH, eval=T, echo=F, cache=F, warning=F, message=F}
...
# Schoenfeld-type residuals from the crr() function was used to plot the 
library(cmprsk)

################################## factor2ind helper function ######################################
if(!require(cmprsk))
...
factor2ind(comp$PtRace, 1)      #Malay as baseline 
factor2ind(comp$Age2, "18-29")  #youngest age group 
factor2ind(comp$PtAssist, 1)    #Complete self-care 
factor2ind(comp$brand, 1)       #Baxter 
factor2ind(comp$dm, "0")          #No diabetes 

x <- cbind(factor2ind(comp$PtRace, 1), factor2ind(comp$Age2, "18-29"), factor2ind(comp$PtAssist, 1), factor2ind(comp$brand, 1), factor2ind(comp$dm, "0")) 

mod.ff  <- crr(ftime = comp$m.time, fstatus = comp$status, x, failcode = 1, cencode = 0) #status=1, peritonitis 
mod.ff2 <- crr(ftime = comp$m.time, fstatus = comp$status, x, failcode = 2, cencode = 0) #status=2, technique failure 

# Schoenfeld-type residual plots for SH of Peritonitis 
mod.ff$res
par(mfrow=c(1,3), mar=c(4.5,4,2,1))
for(j in 1:ncol(mod.ff$res))
  scatter.smooth(mod.ff$uft, mod.ff$res[,j],
                 main = names(mod.ff$coef)[j],
                 xlab = "Failure time",
                 ylab = "Schoenfeld residuals")

# Schoenfeld-type residual plots for SH of Technique failure 
mod.ff2$res
par(mfrow=c(1,3), mar=c(4.5,4,2,1))
for(j in 1:ncol(mod.ff2$res))
  scatter.smooth(mod.ff2$uft, mod.ff2$res[,j],
                 main = names(mod.ff2$coef)[j],
                 xlab = "Failure time",
                 ylab = "Schoenfeld residuals")
...
The PH assumption for the SH models was checked using the Schoenfeld-type residuals tailored for subdistribution hazards. Schoenfeld residual plots indicated the PH assumption was fulfilled for --------- but not for -------(see Supplement) (** need to remove Lucenxia as the PH assumption was broken **). 

**Dignam et al 2012 - the proportionality assumption cannot be simultaneously satisfied for both CHRs and SHRs, but often either model will still provide a reasonably informatinve summary of covariate effects.** 
...
## Effects of PD brands 
Products from Fresenius were significantly associated with an increase in the rate of peritonitis (CSHR 1.68, 95% confidence interval [1.39-2.02]) while it significantly decrease the rate of technique failure (CSHR 0.56, 95% confidence interval [0.35-0.90]). The results were consistent with the cumulative incidence analysis. PD products from Fresenius increased the probability of peritonitis (SH 1.73, 95% confidence interval [1.43-2.08]) while reducing the probability of technique failure (SH 0.47, 95% confidence interval [0.30-0.76]) (see Table XX).

The CSHRs for peritonitis and technique failure showed opposite directions, and the SH for peritonitis was larger than that of CSHR. These results implied that we would observe more peritonitis cases in patients who used Fresenius PD products over a longer observation period. As both the cause-specific and subdistribution analyses were consistent with each other, the effect of using Fresenius PD products on the cumulative incidence of peritonitis was an actual effect and not due to indirect effect on the competing event (technique failure). We are aware that such effect may not necessarily be causal. 

**make a table that looks like Table 2 in Lau et al 2009 - show the changes in csRH1, csRH2, sdRH1 and the relevant interpretation** 

## Effects of diabetes 
Patients who diabetes had a significant increase in the rate of peritonitis (CSHR 1.29, 95% confidence interval [1.05-1.60]) but not statistically signiciant (albeit slightly elevated) rate of technique failure (CSHR 1.29, 95% confidence interval [0.82-2.03]). Diabetes status also had a marginal effect (but not statistically significant) on the cumulative incidence of peritontis (SH 1.23, 95% confidence interval [1.00-1.51]) and no effect on the cumulative incidence of technique failure (SH 1.27, 95% confidence interval [0.83-1.95]). 

Since both CSHRs pointed to the same direction (CSHR_1&2 >1), it was challenging to interpret the cumulative incidence function. As sdRH2 suggested no association (only nominally increased in CSHR with no statistical significance), the effect elicited by diabetes on peritonitis was not a true effect. 


## Effects of assistance required to perform PD 

Compared to patients who were fully independent in performing PD, patients who required partial assistance had a significant increase in peritonitis rate (CSHR 1.32, 95% confidence interval [1.01-1.71]) but a norminal decrease in the rate of technique failure (CSHR 0.67, 95% confidence interval [0.35-1.31]). We also observed a nominally increase in the cumulative incidence of peritonitis (SHR 1.24, 95% confidence interval [0.96 - 1.61]), and a nominal decrease in the cumulative incidence of technique failure (SHR 0.60, 95% confidence interval [0.32-1.13])

Both the CSRH and SHR showed opposite direction for both peritonitis and technique failure. However, due to the absence of statistical significance in SHR1 and SHR2, the effect of partial assistance shown in csHR1 may not be true and could be confounded by other risk factors. 


## Effects of age 
In comparison to the youngest age group, only patients aged between 70-79 years old had a significant increase in the rate of peritonitis (CSHR 1.80, 95% confidence interval [1.05-3.06]). CSHR for technique failure did not demonstrate any statistical significance albeit being nominally lower (CSHR 0.54, 95% confidence interval [0.21-1.35]). The cumulative incidence of peritonitis was significantly increased (SHR 1.85, 95% confidence interval [1.07-3.20]) but nominally lower for technique failure (SHR 0.46, 95% confidence interval [0.19-1.13]). All other age groups showed no significant difference in both the CSHR and SHR for peritonitis or technique failure. 


## other variables 
list out what variables have shown no effect on both CSHR and SHR 


# Discussion 

In this study, we examined two methods to evaluate the influence of four risk factors on peritonitis risks in the presence of a competing event. We showed that the brands of the PD product exerted a true effect on the outcome of interest where patients who used Fresenius PD products had a higher probability of peritonitis. This corresponds to our previous study on a separate patient cohort [@ongRiskPeritonealDialysisRelated2017a]. There were few studies that investigated the relationship between switching PD brands and the timing to first peritonitis and peritonitis rate. Nevertheless, such an association should not be mistaken as a causal effect between Fresenius products and peritonitis. Our work represents an early investigation into the potential effects of risk factors on peritonitis with little attention given to the estimation of causal effects in the presence of competing risks. 
